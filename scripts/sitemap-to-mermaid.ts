#!/usr/bin/env node
/**
 * Generate a drillable Mermaid sitemap:
 * - dist/sitemap/index.mmd  (master)
 * - dist/sitemap/sections/<section>.mmd (per section)
 * - dist/sitemap/assets.json (url->images mapping)
 *
 * Run:
...
for (const key of sortedGroupKeys) {
  ...
  index += `  ${rootId} --> ${segId}\n`;
  // Drill into section viewer page
...
for (const key of sortedGroupKeys) {
  ...
  // Build hierarchy within a section by path up to SECTION_DEPTH
  // sectionRoot -> depth nodes -> pages
  ...
  // Create intermediate nodes for depth navigation
  // map pathKey -> nodeId
  ...
  for (const e of list) {
    ...
    // Only include nodes under this section key
    ...
    if (depthParts.length > 0) {
      // build chain of intermediate nodes
...
console.log(`✅ Wrote: ${OUT_DIR}/index.mmd and ${SECTIONS_DIR}/*.mmd and ${OUT_DIR}/assets.json`);
